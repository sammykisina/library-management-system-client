...
const Select: FC<SelectProps> = ({
  ...
  const getActiveOptionClass = (option: SelectionOption | string) => {
    ...
    if (Array.isArray(selected)) {
      checkIfOptionIsInSelectedArray(option)
        ? (active_option_styles = "bg-primary/50")
        : "";
    } else {
  ...
  return (
    ...
    >
      <div className={`flex items-center justify-between gap-1`}>
        <div className="flex flex-1 gap-1 overflow-x-auto whitespace-nowrap text-sm  capitalize text-secondary scrollbar-hide">
      ...
      >
        {options.map((option, option_index) => (
          <li
            ...
            key={option_index}
            className={`hover:bg-orange/50 w-fit rounded-full  px-2  capitalize hover:font-normal hover:text-primary ${getActiveOptionClass(
              option || ""
            )}`}
          >
