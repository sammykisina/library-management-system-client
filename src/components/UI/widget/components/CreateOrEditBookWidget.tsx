  ...
  useEffect(() => {
    if (globalBook && isEditingBook) {
      reset({
        ...
        yearOfPublish: globalBook?.attributes?.yearOfPublish?.toString(),
          ...
          <div className="flex flex-col gap-y-5 rounded-md border border-primary/10 py-4 px-2">
            ...
            <div className="relative">
              <input
                ...
                {...register("yearOfPublish", { required: true })}
              />
              <label className="inputLabel">Year Of publish</label>

              {errors["yearOfPublish"] && (
                <Error errorMessage={errors["yearOfPublish"].message} />
              )}
            </div>
          </div>
